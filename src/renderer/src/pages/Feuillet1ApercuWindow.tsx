...
import { Feuillet1Doc, Feuillet1PrintCss, type Feuillet1Data } from '@components/documents/Feuillet1'

// ─────────────────────────────────────────────────────────────────────────────
// Aperçu Feuillet N°1 Assurance (Bleu) dans sa PROPRE BrowserWindow
...
//   localStorage('tcit_feuillet1_printed') puis se ferme d'elle-même.
// Paramètres via localStorage('tcit_apercu_feuillet1').
// Fond d'aperçu bleuté : la bande pré-imprimée réelle est BLEUE.
// ─────────────────────────────────────────────────────────────────────────────

const LS_KEY = 'tcit_apercu_feuillet1'
export const FEUILLET1_PRINTED_KEY = 'tcit_feuillet1_printed'
...
const EMPTY_DATA: Feuillet1Data = {
...
export default function Feuillet1ApercuWindow(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      window.print()
      localStorage.setItem(FEUILLET1_PRINTED_KEY, String(params.ts || Date.now()))
    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      {params.autoPrint && (
        <div style={{
          ...
          🖨 Impression du Feuillet N°1 en cours… (bande pré-imprimée bleue 28,2 × 7,51 cm)
        </div>
      )}

      {/* Zone aperçu — bande centrée sur fond gris, teinte bleutée (papier bleu) */}
      <div style={{
        flex: 1, overflow: 'auto', background: '#94A3B8',
        display: 'flex', justifyContent: 'center', alignItems: 'center', padding: 24, minHeight: 0,
      }}>
        <div style={{ boxShadow: '0 4px 20px rgba(0,0,0,0.35)', height: 'fit-content', background: '#EAF4FF', flexShrink: 0 }}>
          <div style={{ mixBlendMode: 'multiply' }}>
            <Feuillet1Doc data={params.data} />
          </div>
        </div>
      </div>
